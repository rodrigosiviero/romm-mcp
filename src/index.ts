 ...
 *
 * Tools:
 *   romm_stats, romm_list_platforms, romm_browse_roms, romm_get_rom,
 *   romm_search_roms, romm_manage_collections, romm_get_collection_roms,
 *   romm_list_firmware, romm_download_rom, romm_update_rom,
 *   romm_rom_notes, romm_manage_saves, romm_manage_states,
 *   romm_system, romm_upload_rom, romm_screenshots
 */
...
server.tool("romm_search_roms", "Search ROMs by name. Returns matching ROMs with basic info. Use this to find specific games.", {
  ...
  // Use browse with search_term param for local search
...
// ════════════════════════════════════════════════════════════
//  Tool 6: romm_manage_collections
// ════════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════
//  Tool 7: romm_get_collection_roms
// ════════════════════════════════════════════════════════════
...
server.tool("romm_rom_notes", "Manage notes attached to a ROM. Actions: list, add, update, delete.", {
  ...
  if (action === "list") {
    ...
    const lines = notes.map((n: any) =>
      `• [${n.id}] **${n.title || "Untitled"}**\n  ${n.content || ""}`
    );
    return { content: [{ type: "text", text: [`📝 **Notes for ROM ${rom_id}**`, "", ...lines].join("\n") }] };
  }

  if (action === "add") {
    const note = await client.addNote(rom_id, { title: title || "Note", content: content || "" });
...
server.tool("romm_system", "System operations: heartbeat check, get config, trigger library scan, or update metadata.", {
  ...
  if (action === "scan") {
    ...
    return {
      content: [{
        type: "text",
        text: `🔍 Library scan triggered. Result: ${JSON.stringify(result)}`,
      }],
    };
  }
  ...
  if (action === "metadata_scan") {
    ...
    return {
      content: [{
        type: "text",
        text: `📝 Metadata scan triggered. Result: ${JSON.stringify(result)}`,
      }],
    };
  }
